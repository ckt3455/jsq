...
class ServiceController extends ApiBaseController
{
    ...
    public function actionDetail()
    {
        ...
        $end_days = $this->end_time($goods->end_time);
        $image=[];
        if($order->image){
        ...
        if($order->worker_id>0){
            $worker=[
                ...
                'worker_image'=>$this->setImg($order->worker_image),
                'worker_phone'=>$order->worker_phone,
                'worker_time'=>date('Y-m-d H:i',$order->worker_time),

            ];
        }
        if($order->status==3 and $order->is_is_evaluate==1){
            $is_evaluate=1;
        }else{
            $is_evaluate=0;
        }
        $data['detail'][] = [
                ...
                'goods_image'=>$this->setImg($order->goods_image),
                'end_days'=>$end_days,
                'image'=>$image,
